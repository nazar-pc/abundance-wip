...
impl From<Box<Proofs<const { PosProof::K }>>> for Box<PosProofs> {
    // NOTE: `no_panic::no_panic` is not applicable here because it fails to parse const generic
    // arguments, the body is a pointer cast that can't panic anyway
    ...
    Self: SupportedKValue,
{
    ...
    pub fn find_quality<'a>(
        ...
        // Iterate just over elements that are matching `first_k_challenge_bits` prefix, a
        // challenge with more than the first `K` bits set simply doesn't have any
        self.table_7
            .buckets()
    ...
    // NOTE: `no_panic::no_panic` is not applicable here because the compiler is unable to prove
    // that `sha2` doesn't panic
    #[cfg(any(feature = "full-chiapos", test))]
    pub fn verify(
        seed: &Seed,
        challenge: &Challenge,
    ...
    >(
        ...
        // At most one match is produced for every two inputs, so there can never be more inputs
        // than `N * 2` here. Limiting them explicitly is what allows the compiler to see that the
        // writes below are within bounds.
        let ys_and_metadata = &ys_and_metadata[..ys_and_metadata.len().min(N * 2)];
            ...
            ys_and_metadata.as_chunks::<2>().0
        {
            ...
            // SAFETY: Inputs are limited to `N * 2` elements above and at most one match is
            // produced for every two of them
            unsafe {
        ...
        // SAFETY: Inputs are limited to `N * 2` elements above and at most one match is produced
        // for every two of them
        unsafe {
            hint::assert_unchecked(next_offset <= N);
        }
