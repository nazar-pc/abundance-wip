...
type Fused = Rv64FusedInstruction<Reg<u64>>;
...
fn test_fuse_upper_addi() {
    ...
    // `addiw` fuses into the same instruction, the sum can't leave the 32-bit range for the
    // fusion to happen at all
    assert_fused(
        Fused::Lui {
            rd: Reg::A0,
            rs1: Reg::Zero,
...
fn test_fuse_slli_srli() {
    ...
    assert_fused(
        ...
        Fused::FusedSlliSrliZextw {
            rd: Reg::A0,
            rs1: Reg::A1,
            rs2: Reg::Zero,
        },
    );
    // A shifted word zero extension stores the shift left over after the word was extracted
    assert_fused(
        Fused::Slli {
            rd: Reg::A0,
            rs1: Reg::A1,
        ...
        Fused::FusedSlliSrliShiftedZextw {
            rd: Reg::A0,
            rs1: Reg::A1,
            rs2: Reg::Zero,
            shamt: 3,
...
fn prepare_bits(state: &mut TestInterpreterState<Fused>) {
    state.regs.write(Reg::A0, 0x0f0f_0f0f_0f0f_0f0f);
