//! Fused instructions of the RISC-V RV32 Zbb extension
...
/// followed by `rori` is the core of every ARX construction, and in a contract built
...
#[instruction(inherit = [Rv32Instruction, Rv32ZbbInstruction])]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Rv32ZbbFusedInstruction<Reg> {
...
const _: () = {
    ...
    assert!(
        size_of::<Rv32ZbbFusedInstruction<Reg<u32>>>() == size_of::<Rv32Instruction<Reg<u32>>>(),
        "Fused instructions must fit into an instruction of the instruction set they extend"
    );
};

#[instruction]
const impl<Reg> Instruction for Rv32ZbbFusedInstruction<Reg>
where
...
#[instruction]
const impl<Reg> FusedInstruction for Rv32ZbbFusedInstruction<Reg>
where
    Reg: [const] Register<Type = u32>,
{
    #[inline(always)]
...
#[instruction]
impl<Reg> fmt::Display for Rv32ZbbFusedInstruction<Reg>
where
...
const impl<Reg> ExecutableInstructionOperands for Rv32ZbbFusedInstruction<Reg> where
    Reg: Register<Type = u32>
{
}

#[instruction_execution]
const impl<Reg, Env> ExecutableInstructionCsr<Env> for Rv32ZbbFusedInstruction<Reg> where
    Reg: Register<Type = u32>
{
}
...
const impl<Reg, Regs, Env, Memory, PC> ExecutableInstruction<Regs, Env, Memory, PC>
    for Rv32ZbbFusedInstruction<Reg>
where
    Reg: [const] Register<Type = u32>,
    ...
    fn execute(
        ...
        match self {
            Self::FusedXorRori {
                ...
                let value = (rs1_value ^ rs2_value).rotate_right(u32::from(shamt & 0x1f));
