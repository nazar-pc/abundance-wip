//! Fused instructions of the base RISC-V RV64 instruction set
...
/// `logic-imm-reg`, `zexth`, `zextw`, `shifted-zextw`, `bfext` and `shift-bit-extract`.
///
...
#[instruction(inherit = [Rv64Instruction])]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Rv64FusedInstruction<Reg> {
    // `addi` + `addi`
    ...
    #[instruction(if = [Auipc, Ld])]
    FusedAuipcLd { rd: Reg, imm: I24 },
    #[instruction(if = [Auipc, Lbu])]
    FusedAuipcLbu { rd: Reg, imm: I24 },
    #[instruction(if = [Auipc, Lhu])]
    FusedAuipcLhu { rd: Reg, imm: I24 },
    #[instruction(if = [Auipc, Lwu])]
    FusedAuipcLwu { rd: Reg, imm: I24 },

    // `lui` + `addi`/`addiw`
    #[instruction(if = [Lui, Addi], if = [Lui, Addiw])]
    ...
    #[instruction(if = [Slli, Srli])]
    FusedSlliSrliZextw { rd: Reg, rs1: Reg },
    #[instruction(if = [Slli, Srli])]
    FusedSlliSrliShiftedZextw { rd: Reg, rs1: Reg, shamt: u8 },
    #[instruction(if = [Slli, Srli])]
    FusedSlliSrli {
        rd: Reg,
        rs1: Reg,
...
const _: () = {
    ...
    assert!(
        size_of::<Rv64FusedInstruction<Reg<u64>>>() == size_of::<Rv64Instruction<Reg<u64>>>(),
        "Fused instructions must fit into an instruction of the instruction set they extend"
    );
};

#[instruction]
const impl<Reg> Instruction for Rv64FusedInstruction<Reg>
where
...
#[instruction]
const impl<Reg> FusedInstruction for Rv64FusedInstruction<Reg>
where
    Reg: [const] Register<Type = u64>,
{
    #[inline(always)]
    ...
    fn fuse(prev: Self, next: Self) -> (Self, Self) {
        match (prev, next) {
            ...
            (
                ...
                (Self::FusedAddSd { rd, rs1, rs2, rs3 }, next)
            }
            ...
            {
                let imm = fused_helpers::combine_upper_immediate(upper.to_i32(), offset);
                (Self::FusedAuipcLwu { rd, imm }, next)
            }

            // `lui` + `addi`/`addiw`, which only differ when the sum leaves the 32-bit range,
            // and a sum that doesn't even fit into the 24 bits below can't have left it
            (
                Self::Lui {
                    rd: prev_rd,
                    imm: upper,
            ...
            (
                ...
                (Self::FusedSlliSrliZextw { rd, rs1 }, next)
            }
            (
                Self::Slli {
                    rd: prev_rd,
            ...
            ) if prev_rd != Reg::ZERO && prev_rd == base && prev_rd == rd && right_shamt < 32 => (
                Self::FusedSlliSrliShiftedZextw {
                    rd,
                    rs1,
                    shamt: 32 - right_shamt,
                },
                next,
            ),
            (
...
#[instruction]
impl<Reg> fmt::Display for Rv64FusedInstruction<Reg>
where
    Reg: fmt::Display + Copy,
{
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::FusedAuipcLd { rd, imm } => write!(
                f,
                "auipc {rd}, 0x{:x}",
                fused_helpers::upper_immediate(imm.to_i32()) >> 12
            ),
            ...
            Self::FusedAuipcLwu { rd, imm } => write!(
                f,
                "auipc {rd}, 0x{:x}",
                fused_helpers::upper_immediate(imm.to_i32()) >> 12
            ),
            ...
            Self::FusedSlliSrliZexth { rd, rs1 } => write!(f, "slli {rd}, {rs1}, 48"),
            Self::FusedSlliSrliZextw { rd, rs1 } => write!(f, "slli {rd}, {rs1}, 32"),
            Self::FusedSlliSrliShiftedZextw { rd, rs1, shamt: _ } => {
                write!(f, "slli {rd}, {rs1}, 32")
            }
            Self::FusedSlliSrli {
                rd,
                rs1,
...
const impl<Reg> ExecutableInstructionOperands for Rv64FusedInstruction<Reg> where
    Reg: Register<Type = u64>
{
}

#[instruction_execution]
const impl<Reg, Env> ExecutableInstructionCsr<Env> for Rv64FusedInstruction<Reg> where
    Reg: Register<Type = u64>
{
}
    ...
    PC: [const] ProgramCounter<Reg::Type, Memory>,
{
    ...
    fn execute(
        ...
        match self {
            ...
            Self::FusedAddiLwu { rd, rs1: _, imm } => {
                let addr = rs1_value.wrapping_add(i64::from(imm).cast_unsigned());
            ...
            Self::FusedAddSb {
                ...
                memory.write(addr, regs.read(rs3) as u8)?;
            ...
            Self::FusedAddSh {
                ...
                let addr = rs1_value.wrapping_add(rs2_value);
                memory.write(addr, regs.read(rs3) as u16)?;
                ExecutionResult::Continue { rd, value: addr }
            }
            Self::FusedAddSw {
                rd,
                ...
                let addr = rs1_value.wrapping_add(rs2_value);
                memory.write(addr, regs.read(rs3) as u32)?;
                ExecutionResult::Continue { rd, value: addr }
            }
            Self::FusedAddSd {
                rd,
                ...
                let addr = rs1_value.wrapping_add(rs2_value);
                memory.write(addr, regs.read(rs3))?;
                ExecutionResult::Continue { rd, value: addr }
            }
            Self::FusedAuipcAddi { rd, imm } => {
                ...
                ExecutionResult::Continue {
                    rd,
                    value: old_pc.wrapping_add(i64::from(imm.to_i32()).cast_unsigned()),
                }
            }
            Self::FusedAuipcLb { rd, imm } => {
                let old_pc = program_counter.old_pc(2 * size_of::<u32>() as u8);
                let addr = old_pc.wrapping_add(i64::from(imm.to_i32()).cast_unsigned());
                let value = i64::from(memory.read::<i8>(addr)?);
                ExecutionResult::Continue {
                    rd,
                    value: value.cast_unsigned(),
            ...
            Self::FusedLuiAddi { rd, imm } => ExecutionResult::Continue {
                rd,
                value: i64::from(imm.to_i32()).cast_unsigned(),
            },
            Self::FusedLuiLb { rd, imm } => {
                let addr = i64::from(imm.to_i32()).cast_unsigned();
                let value = i64::from(memory.read::<i8>(addr)?);
                ExecutionResult::Continue {
                    rd,
                    value: value.cast_unsigned(),
            ...
            Self::FusedAndAndi {
                ...
                let value = (rs1_value & rs2_value) & i64::from(imm).cast_unsigned();
                ExecutionResult::Continue { rd, value }
            }
            Self::FusedAndOri {
                rd,
            ...
            Self::FusedAndiAnd {
                ...
                let value = (rs1_value & i64::from(imm).cast_unsigned()) & rs2_value;
                ExecutionResult::Continue { rd, value }
            }
            Self::FusedAndiOr {
                rd,
            ...
            Self::FusedSlliSrliZextw { rd, rs1: _ } => {
                let value = u64::from(rs1_value as u32);
                ExecutionResult::Continue { rd, value }
            }
            Self::FusedSlliSrliShiftedZextw { rd, rs1: _, shamt } => {
                let value = u64::from(rs1_value as u32) << shamt;
                ExecutionResult::Continue { rd, value }
            }
            Self::FusedSlliSrli {
                rd,
            ...
            Self::FusedSlliwSrliw {
                ...
                let shifted = ((rs1_value as u32) << shamt) >> right_shamt;
                ExecutionResult::Continue {
                    rd,
            ...
            Self::FusedSlliwSraiw {
                ...
                let shifted = ((rs1_value as u32) << shamt).cast_signed() >> right_shamt;
                ExecutionResult::Continue {
                    rd,
