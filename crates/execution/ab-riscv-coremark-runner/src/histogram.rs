...
type State<'a, const BASE: u64, const SIZE: usize> = BasicInterpreterState<
    BasicRegisters<Reg<u64>, true>,
    TimeCsrState,
    GuestMemory<BASE, SIZE>,
    EagerInstructionFetcher<'a>,
>;

#[inline(never)]
pub(crate) fn histogram<const BASE: u64, const SIZE: usize>(
    state: &mut State<'_, BASE, SIZE>,
) -> anyhow::Result<()> {
    ...
    loop {
        ...
        let outcome =
            instruction.execute(rs1rs2_values, regs, env, memory, &mut *instruction_fetcher);
