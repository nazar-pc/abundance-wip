#!/bin/bash
# Builds the Coremark runner and compares the tail-threaded back end against the generic
# interpreter loop. Both live in the same binary, selected by COREMARK_DISPATCH.
#
# The two modes are interleaved round-robin rather than run all-of-one-then-the-other, and the
# reported figure is best-of, because that is the ordering that survives a machine with background
# load. The spread column is as much the point as the timings are: an effect smaller than the spread
...
run_one() {
    ...
    if [ "$mode" = threaded ]; then
        out=$(COREMARK_DISPATCH=1 COREMARK_REPEAT="$REPEAT" ${PIN[@]+"${PIN[@]}"} "$BIN" 2>&1) \
            && status=0 || status=$?
    else
        out=$(env -u COREMARK_DISPATCH COREMARK_REPEAT="$REPEAT" \
            ${PIN[@]+"${PIN[@]}"} "$BIN" 2>&1) && status=0 || status=$?
    fi
...
for mode in generic threaded; do
    out=$(run_one "$mode")
